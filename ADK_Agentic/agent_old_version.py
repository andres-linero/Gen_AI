...
# Pseudocode
def sync_excel_to_qdrant():
    # 1. Load Excel
    df = pd.read_excel("Tracking Orders 2026.xlsx")
    
    # 2. Get existing IDs from Qdrant
    existing_ids = query_qdrant_all_ids()
    
    # 3. Detect changes
    new_rows = df_new[~df_new['ID'].isin(existing_ids)]
    modified_rows = detect_modified_rows(df_new, existing_ids)
    deleted_ids = existing_ids - df_new['ID'].values
    
    # 4. Embed and upload
    for row in new_rows + modified_rows:
        embedding = embed_row(row)
        upsert_to_qdrant(row['ID'], embedding, row.to_dict())
    
    # 5. Delete old rows
    for id in deleted_ids:
        delete_from_qdrant(id)
